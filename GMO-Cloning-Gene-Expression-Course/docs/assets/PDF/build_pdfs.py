...
def title_block(doc_title, subtitle,
    ...
    return [
        ...
        Paragraph("All datasets used in this package are simulated teaching data. "
                  "Wet-lab and animal procedures described here are conceptual; "
                  "institutional biosafety approval and SOPs govern all practice.",
...
# ---- 1. Complete tutorial (all 15 modules + references) --------------------
doc = CourseDoc(os.path.join(BASE, "Complete-Tutorial.pdf"),
                "GMOs, Cloning & Gene Expression - Complete Tutorial")
story = []
story += title_block(
    "Genetically Modified Organisms, Cloning Techniques, and Developmental & "
    "Spatial Gene Expression - a complete university teaching module",
    "For BS/MS Molecular Biology, Biotechnology, Genetics, Bioinformatics, "
    "Biomedical Sciences")
story += toc_block()
...
doc = CourseDoc(os.path.join(BASE, "Lab-Workbook.pdf"),
                "Lab Workbook - GMOs, Cloning & Gene Expression")
story = []
story += title_block(
    "Laboratory and computational workbook for Labs 01-09, with instructor keys "
    "and grading rubrics",
    "Labs 01-09: Simulation, computational and (SOP-governed) teaching-lab formats")
story += toc_block()
for m in sorted(glob.glob(os.path.join(ROOT, "LAB", "*.md"))):
    story += parse_md(m)
    story.append(PageBreak())
story += parse_md(os.path.join(ROOT, "WORKBOOK",
                               "GMO-Cloning-Gene-Expression-Lab-Workbook.md"))
doc.build(story)
...
story = []
story += title_block("2-4 page revision guide: terminology, methods, "
                     "comparisons and common exam mistakes", "Revision aid")
story += parse_md(os.path.join(ROOT, "CHEAT-SHEET",
                "GMO-Cloning-Gene-Expression-Cheat-Sheet.md"))
# add a couple of key figures to fill the visual slot
add_figure(story, "crispr_mechanism.png",
           "CRISPR-Cas9 target recognition and repair outcomes (revision figure).",
           width=11)
add_figure(story, "morphogen_gradient.png",
           "Morphogen gradient with threshold fates (revision figure).", width=11)
doc.build(story)
...
story = []
story += title_block("67 frequently asked questions across 13 categories, "
                     "with scientifically accurate student-friendly answers",
                     "FAQ compendium")
story += toc_block()
story += parse_md(os.path.join(ROOT, "FAQ",
                "GMO-Cloning-Gene-Expression-FAQs.md"))
doc.build(story)
print("FAQs.pdf")

# ---- 5. Assessment ---------------------------------------------------------
doc = CourseDoc(os.path.join(BASE, "Assessment.pdf"),
                "Assessment Package - GMOs, Cloning & Gene Expression")
story = []
story += title_block("50 MCQs, 30 short, 20 long, 40 viva, 12 cases, "
                     "10 data questions - with the full instructor answer key",
                     "Instructor assessment package")
story += toc_block()
for f in ["MCQs.md", "Short-Questions.md", "Long-Questions.md",
          "Viva-Questions.md", "Case-Studies.md", "Answer-Key.md"]:
